...
const Daily = ({ auth }) => {
  ...
  const submit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const { data: response } = await axios.post(route('reports.daily-create'), data);
      if (!response.message) {
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Appointment Daily Report</h2>}
    >
      <Head title="Appointment Daily Report" />

      <div className="grid">
        <div className="col-12 xl:col-12">
          <div className="card">
            <h5>Generate Daily Report of Appoinments</h5>
